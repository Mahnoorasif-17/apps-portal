"""Broker commission automation — Streamlit app module."""
import io
import json
...
# Package imports relative to the central hub
from broker_commission_automation.engine.lookups import load_lookups
...
def show_broker_commission():
    ...
    ALIAS_PATH = os.path.join(BASE_DIR, "data", "aliases.json")
    HISTORY_PATH = os.path.join(BASE_DIR, "data", "run_history.jsonl")
    ...
    with c2:
        st.caption(
            "Data source: file upload now; NetSuite saved-search pull "
            "activates once OAuth credentials are configured in secrets."
        )

    ns_ready = all(
        k in st.secrets
        for k in ("NS_ACCOUNT_ID", "NS_CLIENT_ID", "NS_CERT_ID", "NS_PRIVATE_KEY")
    )
    pull_ns = st.toggle(
        "Pull lookups directly from NetSuite",
        value=False,
        disabled=not ns_ready,
        help=(
            "Requires NetSuite OAuth secrets — see SETUP_NETSUITE.md"
            if not ns_ready
            else "Vendors, opportunities and customers are pulled live via SuiteQL; no lookup uploads needed."
        ),
    )
    ...
    drop_negatives = st.checkbox(
        ...
        "(NetSuite cannot post these bills)",
        value=True,
    )
    use_bundled = st.checkbox(
        "Use bundled June lookups (testing)",
        value=True,
    ...
    def get_lookups():
        if pull_ns:
            from broker_commission_automation.engine.netsuite import (
                NetSuiteClient,
                pull_lookup_frames,
            )
            import io as _io
        ...
        if use_bundled:
            return load_lookups(
                os.path.join(BASE_DIR, "data", "lookups", "vendors.csv"),
                os.path.join(BASE_DIR, "data", "lookups", "opportunities.csv"),
                os.path.join(BASE_DIR, "data", "lookups", "customers.csv"),
            )
        return None
    ...
    def hierarchy_dialog(pending):
        from broker_commission_automation.engine.matching import OverrideStore

        st.caption(
            "The named broker isn't on this group's deal. Choose who "
            "should actually be paid — your choice is remembered for "
            "future runs."
        )
        store = OverrideStore(os.path.join(BASE_DIR, "data", "overrides.json"))
        choices = {}
        for p in pending:
            st.divider()
            st.markdown(
                f"Row says **{p['broker']}** — {p['group_name']} "
    ...
    def review_dialog(pending, matcher):
        st.caption(
            "Accepted matches are remembered for future runs. "
        ...
        for m in pending:
            ...
            options = [
                f"{name}  ({score:.0f}% similar)" for name, score in candidates
            ]
            ...
            if choice.startswith("Reject"):
                ...
                if picked:
                    ...
                    ss.decisions.pop(m.source_name, None)
            else:
                ss.decisions[m.source_name] = candidates[options.index(choice)][0]
        st.divider()
        undecided = [
            m.source_name for m in pending if m.source_name not in ss.decisions
        ]
        if undecided:
            st.warning(
                "Pick a vendor from the search box (or another option) "
                f"for: {', '.join(undecided)}"
            )
        if st.button(
            "Save decisions and continue run",
            type="primary",
    ...
    # --------------------------------------------------------------------- run
    def execute_run():
        from broker_commission_automation.engine.lookups import LookupSchemaError
        ...
        overrides = OverrideStore(os.path.join(BASE_DIR, "data", "overrides.json"))
    ...
    if res is not None and not res.pending_review:
        s = res.stats
        exc_reasons = (
            res.exceptions["Exception Reason"].value_counts().to_dict()
        ...
        if s["output_rows"] and n_hier_miss > s["output_rows"] * 0.10:
            st.warning(
                f"{n_hier_miss} rows couldn't find their group's deal "
        ...
        if abs(s["reconciliation_gap"]) < 0.01:
            st.success(
                "Reconciled — source total = import file + exceptions, "
                "to the cent. Every source dollar is accounted for."
            )
        else:
            st.error(
                f"RECONCILIATION GAP of {s['reconciliation_gap']:,.2f} — "
        ...
        buf = io.StringIO()
        res.output.to_csv(buf, index=False)
        st.download_button(
            f"⬇ Import file — BROKER_COMMISSION_{period}.csv",
            buf.getvalue(),
            f"BROKER_COMMISSION_{period}.csv",
            "text/csv",
        )
        if len(res.exceptions):
            buf2 = io.StringIO()
            res.exceptions.to_csv(buf2, index=False)
            st.download_button(
                f"⬇ Exceptions file ({len(res.exceptions)} rows)",
                buf2.getvalue(),
                f"EXCEPTIONS_{period}.csv",
                "text/csv",
            )
        ...
        st.caption(
            "Upload the final file you produced manually for this same "
            "period. The engine output is compared row by row."
        )
        hist = st.file_uploader(
            "Historical final file (.csv)", type=["csv"], key="hist_final"
        )
        if hist is not None:
            from broker_commission_automation.engine.validate import validate

            report = validate(res.output, pd.read_csv(hist))
            if report.passed:
                st.success(
                    "PASS — classifications and type counts match the "
                    "historical final file."
                )
            else:
                st.warning(
                    "Differences found — review below. Manual-override "
